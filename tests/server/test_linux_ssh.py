...
async def test_poweroff_machine_uses_dbus_when_available():
    """poweroff_machine uses D-Bus alone when it succeeds (no sudo needed)."""
    from familylink_server.services.linux_ssh import poweroff_machine

    mock_conn, mock_cm = _make_ssh_mock(exit_status=0)  # D-Bus call succeeds
    with (
        patch(
            'familylink_server.services.linux_ssh.asyncssh.import_private_key',
            return_value=MagicMock(),
    ...
    mock_conn.run.assert_awaited_once_with(_DBUS_POWEROFF_CMD, check=False)


async def test_poweroff_machine_falls_back_to_sudo_when_dbus_fails():
    """poweroff_machine falls back to sudo systemctl poweroff when D-Bus fails.

    Some distros restrict unauthenticated D-Bus poweroff over polkit.
    """
    from familylink_server.services.linux_ssh import poweroff_machine

    mock_conn, mock_cm = _make_ssh_mock()
    mock_conn.run = AsyncMock(
        side_effect=[MagicMock(exit_status=1), MagicMock(exit_status=0)]
    )
    with (
        patch(
            'familylink_server.services.linux_ssh.asyncssh.import_private_key',
        ...
        await poweroff_machine('host', 22, 'user', 'fake-pem')
    assert mock_conn.run.await_args_list == [
        ((_DBUS_POWEROFF_CMD,), {'check': False}),
        (('sudo systemctl poweroff',), {'check': True}),
    ]
